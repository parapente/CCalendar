...
const reportOptions: Record<string, string> =
    props.report.options === null ? {} : JSON.parse(props.report.options);
...
const onSubmit = () => {
    forms.put(
        route("administrator.report.update", {
            id: props.report.id,
        })
    );
};
...
<template>
    <AppLayout title="Αναφορές">
        ...
        <FormSection class="m-4">
            <template #title>
                <div class="dark:text-white">Επεξεργασία αναφοράς</div>
            </template>
            <template #form>
