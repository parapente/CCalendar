...
const deleteUser = () => {
    ...
    router.delete(route('administrator.user.destroy', [props.user.id, props.type]), {
        onFinish: () => {
...
const toggleActive = () => {
    processing.value = true;
    router.post(route('administrator.user.toggleActive', [props.user.id, props.type]), {}, {
        onFinish: () => {
            processing.value = false;
...
<template>
    ...
    >
        ...
        <div class="flex items-center">
            <Tooltip :message="user.active ? 'Απενεργοποίηση χρήστη' : 'Ενεργοποίηση χρήστη'">
                <button
                    @click="confirmToggle"
                    class="transition ease-in-out duration-300 mx-1 hover:shadow-xl hover:-translate-y-0.5 rounded-md px-3 py-2"
                    :class="user.active ? 'hover:bg-yellow-300' : 'hover:bg-green-300'"
    ...
    <ConfirmationModal :show="showDeleteModal" @close="showDeleteModal = false">
        ...
        <template #content>
            Είστε σίγουροι ότι θέλετε να διαγράψετε τον χρήστη {{ user.name }}; Αυτή η ενέργεια δεν μπορεί να αναιρεθεί.
        </template>
    ...
    <ConfirmationModal :show="showToggleModal" @close="showToggleModal = false">
        <template #title>
            {{ user.active ? 'Απενεργοποίηση' : 'Ενεργοποίηση' }} Χρήστη
        </template>

        <template #content>
            Είστε σίγουροι ότι θέλετε να {{ user.active ? 'απενεργοποιήσετε' : 'ενεργοποιήσετε' }} τον χρήστη {{ user.name }};
        </template>
